...
namespace VSMerlin32.Outline
{
    ...
    {
        List<string> startHideList = new List<string> { "DO", "ELSE" };     //the characters that start the outlining region
        List<string> endHideList = new List<string> { "ELSE", "FIN" };       //the characters that end the outlining region
        ...
        void ReParse()
        {
            ...
            foreach(var line in newSnapshot.Lines)
            {
                ...
                foreach(string startHide in startHideList)
                {
                    foreach(string endHide in endHideList)
                    {
                        //lines that contain a "[" denote the start of a new region.
                        if((regionStart = text.ToUpper().IndexOf(startHide, StringComparison.Ordinal)) != -1)
                        {
                        ...
                        else if((regionStart = text.ToUpper().IndexOf(endHide, StringComparison.Ordinal)) != -1)
                        {
                            int currentLevel = (currentRegion != null) ? currentRegion.Level : 1;
                            int closingLevel;
